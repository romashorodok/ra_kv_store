...
start(_Type, _Args) ->
    {ok, Servers} = application:get_env(ra_kv_store_app, nodes),

    ServerName = element(1, hd(Servers)),
    Nodes = [N || {_, N} <- Servers],
    {ok, ServerReference} =
        application:get_env(ra_kv_store_app, server_reference),
    ...
    {ok, ReconnectInterval} =
        application:get_env(ra_kv_store_app, node_reconnection_interval),
    {ok, _} =
    ...
    {ok, Port} = application:get_env(ra_kv_store_app, port),
